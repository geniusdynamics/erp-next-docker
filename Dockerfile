...
# Copy Node.js from official image (no NVM needed)
COPY --from=node:${NODE_VERSION}-alpine /usr/local/bin/node /usr/local/bin/node
COPY --from=node:${NODE_VERSION}-alpine /usr/local/bin/npm /usr/local/bin/npm
